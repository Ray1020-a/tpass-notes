...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ notes: filtered, tags: tagsResult.rows.map((row: TagRow) => row.name), stats });
}
...
export async function POST(request: Request) {
  ...
  await initDb();
  const noteResult = await query<InsertNoteRow>(`INSERT INTO notes (title, content_type, owner_email, owner_name, published, latest_content) VALUES ($1, $2, $3, $4, true, '') RETURNING id`, [title, contentType, session.email, session.name]);
  ...
  for (const tagName of tagNames) {
    ...
    if (!tagId) {
      const insertTag = await query<InsertNoteRow>(`INSERT INTO tags (name) VALUES ($1) RETURNING id`, [tagName]);
